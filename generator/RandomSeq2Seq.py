    ...
    if use_teaching_forcing:
        for di in range(target_length):
            decoder_output, decoder_hidden, decoder_attention = decoder(decoder_input,
                                                     decoder_hidden,encoder_outputs, "cuda")
...
def trainIters(encoder,
    ...
    training_pairs = [
        tensorsFromPair(voc, randomPairs[i][2], randomPairs[i][0], "random")
...
def evaluate(encoder, decoder, sentence, max_length=MAX_LENGTH):
    with torch.no_grad():
        input_tensors = [
            tensorFromSentence(voc, sentence[random.randint(0,len(sentence)-1)])
        ]
        input_size = len(input_tensors)
...
trainIters(encoder1, attn_decoder1, 1000000)

encoder_save_path = "Randomencoder3.pth"
decoder_save_path = "Randomdecoder3.pth"
