...
=======
def trainIters(encoder, decoder, n_iter, print_every = 10, plot_every=100, learning_rate = 0.01):
>>>>>>> 88cd809551d2f7302bee3f26c8eeccf66e061530
    '''
...
=======
    ...
    randomPairs = [random.choice(pairs) for i in range(n_iter)]
    training_pairs = [tensorsFromPair(voc,randomPairs[i][2],randomPairs[i][1])  for i in range(n_iter)]
    criterion = nn.NLLLoss()
...
torch.save(encoder1, current_dir+'\encoder1.pth')
torch.save(attn_decoder1, current_dir+"\\"+'attn_decoder1.pth')
model1 = torch.load(current_dir+'\encoder1.pth')
model2 = torch.load(current_dir+"\\"+'attn_decoder1.pth')
